...
{
    public class EmployeeServiceProxy : IOffeEmployeeService

    {
        ...
        public async Task UpdateAsync(int id, int employeeId)
        {
            var json = JsonSerializer.Serialize(employeeId);
            var data = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await Client.PutAsync($"{_employeeRequestUri}/offer/{id}", data).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();
        }
        ...
        public async Task AddEmployeeOffer(OfferEmployeeDTO offerEmployee)
        {
            ...
            var data = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await Client.PostAsync($"{_employeeRequestUri}/offer", data).ConfigureAwait(false);
            response.EnsureSuccessStatusCode();
        }
        public async Task<IEnumerable<OfferEmployeeDTO>> GetOfferEmployeeAsync(int id)
        {
            var response = await Client.GetAsync($"{_employeeRequestUri}/offer/employeeoffer/{id}").ConfigureAwait(false);
            ...
            return await JsonSerializer.DeserializeAsync<IEnumerable<OfferEmployeeDTO>>(stream, options).ConfigureAwait(false); //returnere en strøm af itemDTO'er og de er tællelige.
        }

        public async Task<IEnumerable<OfferEmployeeDTO>> GetEmployeesNotOnOfferAsync(int id)
        {
            var response = await Client.GetAsync($"{_employeeRequestUri}/offer/employeenooffer/{id}").ConfigureAwait(false);
